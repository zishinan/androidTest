...
public class RegisterActivity extends CommonActivity implements OnClickListener {
	...
	public void onClick(View v) {
			...
			if(usernameEditText.getText().toString().length() < 5 || usernameEditText.getText().toString() == null || "".equals(usernameEditText.getText().toString().trim())){
				Toast toast = Toast.makeText(this, "帐号格式不对", Toast.LENGTH_SHORT);
		    	toast.setGravity(Gravity.CENTER, 0, 0);
		    	toast.show();
			} else if(passwordEditText.getText().toString() == null || "".equals(passwordEditText.getText().toString().trim())){
				Toast toast = Toast.makeText(this, "密码格式不对", Toast.LENGTH_SHORT);
		    	toast.setGravity(Gravity.CENTER, 0, 0);
		    	toast.show();
			} else if(verifyEditText.getText().toString() == null || "".equals(verifyEditText.getText().toString().trim())){
				Toast toast = Toast.makeText(this, "确认密码格式不对", Toast.LENGTH_SHORT);
		    	toast.setGravity(Gravity.CENTER, 0, 0);
		    	toast.show();
			} else if ( passwordEditText.getText().toString().trim().equals(verifyEditText.getText().toString().trim()) ){
