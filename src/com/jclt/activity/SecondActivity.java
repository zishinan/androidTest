...
import com.jclt.server.ResultData;
...
//				true);
	...
	private List<Map<String, Object>> getDate() {
		...
		List<Dir> mainDirs = ResultData.getMainDirs();
		Log.e("mylog", mainDirs.size()+"");
		for (Dir dir : mainDirs) {
			Map<String, Object> maplitong = new HashMap<String, Object>();
			maplitong.put("text", dir.getName());
			Log.e("mylog", dir.getName());
			System.out.println(dir.getName());
			maplitong.put("img", R.drawable.toright_mark);
			maplitong.put("img_pre", R.drawable.paopao);
			listLitong.add(maplitong);
		}
		SecondActivity.this.list = listLitong;
		return listLitong;
	}
...
//	static final String[] TYPE = { "男鞋", "女鞋", "更多分类", "新品", "品牌馆", "每日精品" };
	...
	class listViewLitongOnclickListener implements OnItemClickListener {
		...
		public void onItemClick(AdapterView<?> list, View view, int position,
				long id) {
			String name = (String) SecondActivity.this.list.get(position).get("text");
			Log.e("mylog", name);
			Long dirId = ResultData.getDirIdByName(name);
			List<Smalldir> allSmalldirs = ResultData.getSmalldirs();
			List<Smalldir> selectSmalldirs = new ArrayList<Smalldir>();
			for (Smalldir smalldir : allSmalldirs) {
				if(dirId.equals(smalldir.getDir().getId())){
					selectSmalldirs.add(smalldir);
				}
			}
			ResultData.setSelectSmalldirs(selectSmalldirs);
				intent.setClass(SecondActivity.this, SmallDirActivity.class);
